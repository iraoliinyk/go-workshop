...
// decodeJSON reads the body into dst and reports whether it succeeded. It rejects
// unknown fields, so a typo in a client payload is a 400 instead of a field that is
// quietly ignored.
...
// error type without adding a branch here gives a 500, which is the safe default.
//
// Every branch logs the full error while the response keeps a generic message: the
// client must not learn which check failed, but the operator reading the log needs to.
func (a *API) writeError(w http.ResponseWriter, r *http.Request, err error) {
	...
	if e, ok := errors.AsType[*apperrors.UnauthorizedError](err); ok {
		w.Header().Set("WWW-Authenticate", `Bearer realm="ch-4"`)
...
// logError writes the one-line "METHOD PATH -> status: detail" form used by every
// mapped branch above, tagged with the error code and the request ID.
//
// The level follows who is at fault: a 4xx is the caller's problem and only warns, so
// PROD drops it, while a 5xx is this server's problem and is written in both modes.
